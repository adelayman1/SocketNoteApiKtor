...
class RegisterUseCase constructor(var userRepository: UserRepository) {
    suspend operator fun invoke(name: String, email: String, password: String): BaseResponse<UserModel> {
        return try {
            ...
            val registerResult = userRepository.register(email, password, name)
            return if (registerResult != null) BaseResponse.SuccessResponse(
